...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub2 = n.subscribe("/odom",100,&CurrentStatesListener::callback,&mlistener2);

    //创建控制量发布对象
    ros::Publisher controls_pub =n.advertise<geometry_msgs::Twist>("/cmd_vel",100);

    //创建预测轨迹发布对象
    ros::Publisher path_pub =n.advertise<nav_msgs::Path>("/predict_trajectory",100);

    //创建rviz目标点可视化发布对象
    ros::Publisher marker_pub =n.advertise<visualization_msgs::Marker>("/goal_marker",100);
    ...
    while(ros::ok())
    {
        ...
        p1.z = 0;
        p2.x = mlistener1.m_goal_states[0]+2*std::cos(mlistener1.m_goal_states[2]);
        p2.y = mlistener1.m_goal_states[1]+2*std::sin(mlistener1.m_goal_states[2]);
        p2.z = 0;
